...
- **Plan / vibe / goal / loop / memory (đợt 2):** plan mode submit được (`xd://propose` cần proposal handler — trước đó throw nên không bao giờ approve được): adapter đăng ký handler, đẩy `plan-proposal` event, panel review Approve+execute / Approve+keep / Refine. Vibe mode giờ thật: swap toolset sang `read`+`todo`+`vibe_*`, mở owner scope của `VibeSessionRegistry`, `killAll` + restore toolset khi tắt (verify: worker `fast` chạy `echo worker-ok` qua `vibe_spawn`/`vibe_wait`, tắt thì tool biến mất). Goal: `/goal budget` sửa tại chỗ qua `goalRuntime.onBudgetMutated` (trước đó `set` làm reset `tokensUsed` + đổi id), `timeUsedSeconds` + event `goal_updated` đẩy realtime. `/loop` có đủ: limit kiểu `10`/`10m`/`1h30m`, `loop.mode` prompt/compact/reset, pause/stop, strip ở UI. Memory session-scoped với đủ op `status/view/stats/diagnose/queue/clear/enqueue/search`, đổi backend gọi `applyMemoryBackend()` cho session đang chạy. Plan/vibe/goal loại trừ nhau (409 như TUI).
...
- **Plan mode phần sâu** — chưa: đổi model theo role `plan` khi vào plan mode + restore khi ra, snapshot/restore toolset, `/plan-review` mở lại review, `plan.defaultOnStartup`.
- **Memory** — chưa: mental models của Hindsight (`/memory mm …`), browse/sửa file `memory://` trong UI (read path đã có), `guided-goal`.
