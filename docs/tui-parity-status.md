...
| `/browser`, `/computer` | tab Browser: mode Web/Desktop — open/url/title/ariaSnapshot/screenshot/click/type (Web), capabilities/displays/windows/screenshot (Desktop); row của ariaSnapshot click được (`aria-ref=eN`) | 🟡 | prelude passthrough `POST /api/sessions/:id/{browser,computer}` (gate `browser.enabled`/`computer.enabled` đọc **live** settings). **Verify lại 2026-09-22 trên server cách ly (`PI_CODING_AGENT_DIR` riêng, `computer.enabled=true`):** `displays` → 1 display "Built-in Retina Display" 1680×1050 @2x, `backend: quartz`, **`capturePermission: granted`**, `inputPermission: denied`, `axPermission: denied`; `windows` → danh sách cửa sổ thật (Chrome, PostgresMenuHelper…); `screenshot` → PNG **3360×2100** (đã mở xem: đúng màn hình thật, Terminal đang chạy session này); `capabilities` → `Computer capabilities unavailable`; `click` → `PermissionDenied: macOS Accessibility permission is required for native input`. **Còn 🟡 đúng 1 điều kiện:** cấp **Accessibility** (+ có thể **Input Monitoring**) cho app host server (Apple_Terminal khi chạy từ shell, hoặc `Grove.app`) → chạy lại `click`/`type` + `/browser` rồi chốt ✅ · commit `8dbc504` |
