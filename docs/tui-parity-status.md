...
| `/browser`, `/computer` | tab Browser: mode Web/Desktop — open/url/title/ariaSnapshot/screenshot/click/type (Web), capabilities/displays/windows/screenshot (Desktop); row của ariaSnapshot click được (`aria-ref=eN`) | ✅ | prelude passthrough `POST /api/sessions/:id/{browser,computer}` (gate `browser.enabled`/`computer.enabled` đọc **live** settings). **Verify 2026-09-23 trên máy thật** (server cách ly `PI_CODING_AGENT_DIR` + session tạm, đúng chuỗi lệnh pane gọi; quyền đã cấp: `capturePermission`/`inputPermission`/`axPermission` = **`granted`**): `displays` → 1 "Built-in Retina Display" 1680×1050 @2x, `backend: quartz`; `windows` → danh sách cửa sổ thật (Chrome, PostgresMenuHelper…); `screenshot` → PNG **3360×2100** (ảnh trả về client); `capabilities` sau lần chạy đầu → `{"capture":true,"input":true,"ax":true,"backgroundWindowInput":true,"deliveryModes":["background","foreground"]}`; **input thật vào cửa sổ Calculator bằng background delivery** (không cướp focus): `window.screenshot` 396×700 → `click 198,105` **200** → `type "123"` **200** → AX đọc lại `statictext [ref=e9]: "−123"`; `move` desktop **200**; Web: `open https://example.com/` (headless, title `Example Domain`) → `ariaSnapshot` (refs `e2…e6`) → `click aria-ref=e2` **200**. Trước đó (2026-09-22) `click`/`type` trả `PermissionDenied: macOS Accessibility permission is required for native input` — hết khi cấp quyền (xem §9) · commit `8dbc504`, `164d48c` |
...
| `/install`, `/marketplace`, `/reload-plugins` (install/enable/update) | tab Plugins: toggle **Installed | Browse** — Browse liệt kê plugin kèm marketplace nguồn + Install; Installed có Enable/Disable, Uninstall, badge **update** + Upgrade | ✅ | Bọc `MarketplaceManager` của SDK (không cần TTY như kết luận cũ): 7 route `/api/marketplace/*` + `/api/plugins/marketplace/*`. Verify: adapter 6/6, route 8/8, HTTP smoke 200/400/502, browser chạy trọn vòng install → update → disable → uninstall trên agent dir cách ly; `bun run check` xanh + e2e 15/15 · commit `962ab83` |
| `/smithery-search` | — | ⬜ | cần nguồn registry riêng (MarketplaceManager không bao gồm) |
...
| Providers (status/auth mode) | ProvidersPane: status + `auth: key/oauth/keyless/none` + **Login/Logout** (dialog URL → paste code), cột Action chỉ hiện cho provider có `login` | ✅ | Flow chạy **trong gateway** qua `AuthStorage.login(provider, ctrl)`; web chỉ thấy attempt (`running/needs-input/complete/failed/cancelled` + `auth.url` + prompt + `identity`) — **không** credential nào qua web. Route: `POST /api/providers/:id/login`, `GET /api/providers/logins/:id`, `POST …/input`, `POST …/cancel`, `DELETE /api/providers/:id`. Verify: adapter `provider-login.test.ts` **4/4** (URL→prompt→credential, cancel không ghi credential, logout huỷ attempt đang chờ, provider/attempt lạ), route **4/4** (start→needs-input→input→complete trên provider giả, 404/400 qua `errorToStatus`, logout trả list mới), HTTP thật (server cách ly): `GET /api/providers` → 81 provider / **70 loginable**; `POST …/not-a-provider/login` → **400** `provider … does not support OAuth login`; `GET/POST …/logins/nope*` → **404** `unknown login attempt: nope`; body thiếu `value` → **400** zod; e2e `provider login` **17/17**. **Browser thật** (dev stack, agent dir cách ly): bấm Login `anthropic` → dialog hiện URL thật `https://claude.ai/oauth/authorize?…redirect_uri=http%3A%2F%2Flocalhost%3A54545%2Fcallback…code_challenge_method=S256` + instruction của SDK + progress `Waiting for browser authentication...`, rồi provider hỏi code → dialog hiện prompt paste; **Cancel** → `Sign-in cancelled.`, đóng dialog, row vẫn `NONE Login` (không credential nào được ghi). Non-goal giữ nguyên: API-key entry vẫn ở Settings · commits `2dfe8e1`, `7041c04`, `4d84942` |
...
- **`/computer` cần 3 quyền macOS** cho app host server (Apple_Terminal khi chạy từ shell, hoặc `Grove.app`): Screen Recording (capture), **Accessibility** (native input + AX), tuỳ máy thêm Input Monitoring. Thiếu quyền thì `displays`/`windows`/`screenshot` vẫn chạy nếu có capture, còn `click`/`type` trả `PermissionDenied: macOS Accessibility permission is required for native input`. Trạng thái quyền đọc được ngay trong `details` của **mọi** call (`capturePermission`/`inputPermission`/`axPermission`).
- **`capabilities` trước lần chạy đầu** → `Computer capabilities unavailable`: SDK chỉ biết caps từ payload của một run (TUI cũng vậy), nên nút Capabilities ở pane khi session vừa mở là vô nghĩa — gọi `displays`/`screenshot` trước thì nút mới có nội dung.
- **Input theo toạ độ phải có screenshot trước**: `click(x,y)` tính bằng pixel của **lần screenshot gần nhất cùng target**; chưa chụp → `InvalidCoordinateFrame: no capture of '<window>' yet`.
- **Keystroke `delivery:"background"` cần app chỉ có 1 cửa sổ**: macOS gửi phím tới cửa sổ *key* của app, nên app nhiều cửa sổ → `BackgroundUnavailable: window <id> is one of N windows in its application…`; chọn `delivery:"foreground"` (cướp focus) hoặc dùng AX (`ref` + `setValue`).
...
| 13 | minor | Providers pane chờ **model catalog 506 kB** (`/api/models` 0.4–6s) nên skeleton chiếm cả pane ~5s | **ĐÃ SỬA** — `/api/models` nén gzip khi client nhận + `ETag`/304 (`routes/respond.ts`, `respond.test.ts` 4/4): đo trên server tươi **509 853 B → 31 922 B (16×)**, `If-None-Match` → 304 | commit `9f88388` |
...
| 18 | minor | `GET /api/sessions/:id/export` trên session dài trả **42 MB** trong một response (UI tải trọn vào bộ nhớ) | **ĐÃ SỬA** — `GET …/export?as=file` stream từ file tạm (`Content-Disposition: attachment`, xoá temp dir sau khi ghi), route JSON chặn > 8 MB bằng 413; web tải qua browser (không tạo Blob) | verify: `share.test.ts` 3/3 (stream/thiếu file/413) · e2e `export streams a document for a session that has a journal` · commit `2c58605` |
...
**Chưa phủ:** luồng plan propose→approve (nỗ lực đầu bị nhiễu do draft cũ, chưa chạy lại sạch), hub spawn/steer/inbox (tốn model turn), collab live, debug launch/attach, security scan thật, bash `pty:true`, `conflicts/resolve`, import session ngoài. (`/browser` + `/computer` đã phủ ngày 2026-09-23 — xem §5.)

---

## Changelog

- 2026-09-23 · **Provider OAuth login/logout trong web** (dòng §7 `Providers` 🟡 → ✅): attempt state machine ở adapter (`provider-login.ts`) bọc `AuthStorage.login`, 5 route, dialog URL/paste-code ở ProvidersPane thay cho hướng dẫn `omp login …` trong terminal; `ProviderEntry.login` cho biết provider nào login được (81 provider / 70 loginable). Verify: adapter 4/4, route 4/4, HTTP thật 400/404 đúng, e2e 17/17, browser thật với `anthropic` (URL PKCE + prompt paste code + cancel không ghi credential). Bỏ test `ui-invariants` pin câu chữ dialog cũ (test assert source text) · commits `2dfe8e1`, `7041c04`, `4d84942`

- 2026-09-23 · **Đính chính lý do dòng `Providers` 🟡**: không phải "SDK không export device-code/cần TTY" — `@oh-my-pi/pi-ai/oauth` có `pollOAuthDeviceCodeFlow` + callback server loopback + PKCE + `AuthStorage.login/logout`, và RPC mode của SDK đã bridge headless (`rpc-mode.ts:1501,1511`). Giữ 🟡 vì ngoài scope, kèm đường làm cụ thể (2 route + dialog URL/paste-code) · commit `6df5bd6`
