...
| 5 | major | Process/broker: `ps` hợp lệ **12.8s**; scope chưa có broker sống → `500 connect ENOENT …/broker.sock` sau **35.4s/23.3s** (child broker chết êm, SDK `stderr:"ignore"`); daemon đã `stop` vẫn nằm trong `ps` | MỞ — nguồn là broker: cold ~9.9s, warm 1.27s; deadline 20s + 503 đã có | `POST …/process {"op":"ps"}` → `200 12.79s`; scope mới → `500 Failed to start daemon broker: connect ENOENT …` (`~/.omp/run/daemons/3cf0e54b…` chỉ có `broker.token`+`scope.json`) · **đối soát 2026-09-22:** `ps` cold 9.95s / warm 1.27s, `describe nope` 400 sau 1.21s warm |
| 6 | major | `POST /thinking` **không validate** level (nhận mọi chuỗi) và **không có GET** để đọc lại → UI có thể hiển thị mức không tồn tại | MỘT NỬA — validate xong (`banana` → 400 kèm vocab); thiếu `GET /thinking` (404) | `POST …/thinking {"level":"banana"}` → `200 {"thinking":"banana"}`; `GET …/thinking` → `404 {"error":"not found"}` · **đối soát:** `banana` → `400 unknown thinking level: banana (expected off | minimal | …)`; `GET /thinking` → `404` |
...
| 9 | minor | `POST /move` **tự tạo thư mục đích** (side effect không hỏi; `/workspace/dirs` thì validate) | MỞ — `POST /move {"cwd":"/tmp/x"}` → 200 và thư mục được tạo | `POST …/move {"cwd":"/tmp/grove-audit-moved-new"}` → `200 {"ok":true}` và thư mục xuất hiện · **đối soát:** tạo thư mục thật (`/tmp/grove-defect9` xuất hiện sau lệnh) |
...
| 12 | minor | Tạo session **flaky**: 1 lần `500 {"error":"Agent \"Main\" was replaced during session initialization."}`, retry cùng body thì OK | THEO DÕI — 1 lần đo: 200 trong 0.51s; e2e có test tạo song song | agent, 1/2 lần |
