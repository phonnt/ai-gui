...
| `/ssh` (quản lý host) | section **SSH hosts** trong tab Explorer: chọn scope user/project, form name+host, xoá từng host | ✅ | adapter bọc `ssh/config-writer` của SDK (`addSSHHost`/`removeSSHHost`/`listSSHHosts`/`validateHostName`) + `getSSHConfigPath`; route `GET/POST/DELETE /api/sessions/:id/ssh`. Verify live (dev server :8787): list rỗng → `{"hosts":[]}`, add → `{"hosts":["probe"]}`, trùng → 400 `ssh host already exists: probe`, body thiếu → 400 nêu tên field, delete → `{"hosts":[]}`; config ghi vào `<cwd>/.omp/ssh.json`. e2e `the explorer manages ssh hosts end to end` (thêm qua UI → có trong server → xoá) · commit `$(h)` |
| `/git` | — | ⬜ | git dùng qua bash (Task 4 của plan `2026-09-22-parity-closeout`) |
