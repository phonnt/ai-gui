...
| Task | Commit | Kết quả |
|---|---|---|
| 1 | `2dfe8e1` | `provider-login.ts` (attempt state machine) + `ProviderEntry.login` + `oauthLoginProviderIds()` trong `settings-catalog.ts`; test **4/4**. **Deviation:** thêm `providerLoginSettled(attemptId)` (await trạng thái terminal) — không có trong Interfaces của plan, nhưng là cách duy nhất để test/route chờ flow kết thúc **không dùng wall-clock** (`Bun.sleep` bị rule cấm trong test); nó cũng là primitive cho long-poll sau này. **Deviation:** test dùng scope tường minh (`{cwd, agentDir}` temp) chứ không đổi `PI_CODING_AGENT_DIR` — SDK resolve agent dir **một lần** lúc import (`pi-utils/dirs.ts:433`), nên đổi env giữa test không có tác dụng. |
| 2 | `7041c04` | Protocol: `login` trên `ProviderEntrySchema` + 4 schema/DTO cho attempt; `routes/provider-login.ts` (5 route) + path constants/dispatch; test **4/4**, `smoke:server` OK, HTTP thật: 81 provider/**70 loginable**, 400 cho provider lạ, 404 cho attempt lạ, 400 zod khi thiếu `value`. **Deviation:** route để lỗi typed của adapter **nổi lên** (server map bằng `errorToStatus`) thay vì bọc `HttpError` như `prelude.ts` — giống nhánh catalog sẵn có, và test assert qua chính classifier đó. **Deviation:** `apps/server` thêm devDeps `@oh-my-pi/pi-utils` + `@oh-my-pi/pi-ai` (chỉ dùng trong test) để route test trỏ agent dir sang temp (`setAgentDir`) và đăng ký provider giả — không có cách nào khác để test route mà **không** ghi credential vào store thật. |
| 3 | `4d84942` | Web: 5 hàm rest + `useProviderLogin`/`useProviderLogout` + `ProviderLoginDialog` + ProvidersPane (Login/Logout, xoá hướng dẫn terminal); e2e spec mới. `bun run check` xanh, `bun run e2e` **17/17**. **Deviation:** stub e2e có trạng thái (needs-input → complete sau khi submit) — stub tĩnh làm dialog nhảy ngược giữa assert. **Deviation:** xoá test `ui-invariants` "the connect dialog keeps its instructions and login command" — nó assert **source text** (`OMP sign-in opens in your browser`, `omp login {connectId}`); copy đó không còn tồn tại và hành vi thật giờ có spec riêng. |
| 4 | _pending_ | Browser thật (dev stack, agent dir cách ly): Login `anthropic` → URL PKCE thật + instruction + progress → provider hỏi code → prompt paste hiện ra; Cancel → `Sign-in cancelled.`, row không đổi (`NONE Login`, không credential). Parity row §7 `Providers` 🟡 → ✅ + changelog. |

**Review Focus đã phủ:** #1 provider hỏi input trước auth URL (test adapter + route dùng provider giả hỏi prompt ngay; browser thật cũng vào nhánh prompt) · #2 provider id không hỗ trợ OAuth → 400 (adapter + route + HTTP) · #3 attempt lạ/hết TTL → 404 (route + HTTP; TTL prune trong `pruneExpired`) · #4 cancel giữa flow → `cancelled`, **không** credential (test adapter dùng `providerLoginSettled` + `providersList`) · #5 logout khi attempt đang chạy → attempt `cancelled`, credential không hồi sinh (test adapter + route).
