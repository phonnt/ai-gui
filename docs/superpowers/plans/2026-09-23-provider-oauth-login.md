...
| 4 | `20adae8` | Browser thật (dev stack, agent dir cách ly): Login `anthropic` → URL PKCE thật + instruction + progress → provider hỏi code → prompt paste hiện ra; Cancel → `Sign-in cancelled.`, row không đổi (`NONE Login`, không credential). Parity row §7 `Providers` 🟡 → ✅ + changelog. |
