...
**Đối soát sau Task 9 (2026-09-22):** bản Task 9 làm theo regex ở Step 1 — vốn **mâu thuẫn với Ruling A** đã chốt ở pre-flight — nên đã cấm `border-link`/`border-warning` và đổi 2 hộp alert sang 0.5px. Đã sửa lại theo Ruling A (`border border-link` / `border border-warning` giữ 1px, guard chỉ cấm form chrome, utility `panel-link`/`panel-warning` xoá vì chỉ phục vụ markup đã revert). Verify: guard 16 pass, 6 mẫu chrome vẫn bị bắt + 2 mẫu alert không bị bắt, `bun run check` xanh, `bun run e2e` 12/12. Commit `1362363`.
