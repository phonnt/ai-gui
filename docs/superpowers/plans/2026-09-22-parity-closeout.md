...
| 3 | `23c040a` | Thêm dep `@oh-my-pi/pi-utils`. Adapter: `listSshHosts/addSshHost/removeSshHost` bọc `ssh/config-writer` + `getSSHConfigPath`; khai 3 method trên `AgentRuntime`; route `apps/server/src/routes/ssh.ts` (GET/POST/DELETE) + `SSH_PATH`; web: `rest/settings.ts` + `hooks/settings.ts` + `SshHostsSection.tsx` (scope user/project, form, xoá) gắn trong ExplorerPane. Test: adapter **3/3** (thêm/list/xoá, tên sai, trùng, config hỏng — đã chứng minh đỏ khi bỏ catch), route **4/4**, e2e **14/14**. Live :8787: list rỗng → add → trùng 400 → body thiếu 400 → delete sạch; config ở `<cwd>/.omp/ssh.json`. **Ruling:** không thêm lệnh palette `/ssh` — palette đã có lệnh chuyển tab tới Explorer (`onTab`), thêm đường thứ hai tới cùng tab là trọng lượng vô ích; cost nếu sai: gõ `/ssh` trong composer không chạy gì (parity row ghi rõ người dùng vào Explorer). **Ruling nhỏ:** field SDK là `username` chứ không phải `user` — API web vẫn nhận `user` cho gọn. |
| 4 | `c1fd0a4` | Adapter `gitStatus`/`gitDiff` qua `runBashImpl` + `parseNumstat` (SDK), route `git.ts` (status/diff), web `GitSection.tsx` trong Explorer + `rest/hooks` git. Test: adapter **2/2** (branch/file/insertions trong repo tạm; ngoài repo trả rỗng — timeout 30s vì 2 call tool ~3s), route **3/3**, e2e **15/15**. **Ruling:** không kéo `@oh-my-pi/pi-natives/vcs` (native addon chưa khai báo, ảnh hưởng đóng gói desktop) — dùng bash + parser SDK; cost nếu sai: chậm hơn ~1,5s/lần gọi so với native. **Ruling:** diff hiển thị inline trong section (không mở editor pane) — cost nếu sai: hai chỗ hiển thị diff, không hỏng. **Hai phát hiện từ TDD (đã sửa trong commit này):** (1) `BashResult` dùng `output`/`exitCode`, không phải `stdout`; (2) session do adapter tạo **chưa đăng ký cwd cho tools** → mọi call tool lỗi `session not found`; `createSession` giờ gọi `setSessionCwd`, và adapter đăng ký tool table factory trong constructor (`createSessionTools()`) để embedder không cần server boot. **Ruling:** e2e git đọc branch bằng `git rev-parse` tại chỗ thay vì round-trip API (API chậm >15s khi suite chạy đủ) + timeout 30s — cost nếu sai: spec che mất route chậm (route vẫn có test riêng). |
| 5 | `962ab83` | Adapter bọc `MarketplaceManager` (7 method + `resolveOrDefaultProjectRegistryPath` cho scope project + `clearPluginRootsCache` để install hiện ngay); route mới `apps/server/src/routes/plugins.ts` (7 route marketplace, đồng thời dồn listPlugins/listExtensions về đây); protocol 11 schema; web PluginsSection thêm **Installed | Browse** + Update/Upgrade. Test: adapter **6/6**, route **8/8**, `bun run check` xanh, e2e **15/15**; HTTP smoke 200/400/502 + browser chạy trọn vòng install → update → disable → uninstall (agent dir cách ly, registry thật không bị đụng). **Ruling:** `listMarketplacePlugins` gắn thêm `marketplace` nguồn cho mỗi entry (SDK không trả) — nếu không thì lệnh install không xác định được nguồn; cost nếu sai: sai tên field so với SDK nếu SDK đổi. **Ruling:** giao task này cho subagent (context của phiên đã dài) — cost nếu sai: chất lượng brief phụ thuộc vào plan, đã verify lại bằng test + smoke + browser. **Ruling:** parity doc sửa kết luận cũ "cần package-manager TTY". |
| 6 | `40ce523` | Lệnh palette `Rewind (branch from here)`; quy tắc "user message mới nhất" tách từ `Transcript.tsx` sang `turns.ts` (`lastUserMessageId`) để transcript và lệnh không lệch nhau. Test: `turns.test.ts` (RED→GREEN, 2 mới) + `chat-commands.test.ts` (2, **đã chứng minh đỏ khi tạm bỏ lệnh**) → `bun test turns.test.ts chat-commands.test.ts` **10/10**. **Ruling:** không thêm lệnh palette riêng cho `/ssh`/marketplace (tab đã tới được bằng lệnh chuyển tab) nhưng `/rewind` thì thêm vì web không có đường bàn phím nào khác — cost nếu sai: một mục palette thừa. |
| 7 | `b9449c6` | `log-cache.ts` (thuần, clock inject) + wire vào `processAction` op `logs`: giữ window 2s, trả slice theo cursor riêng của adapter; cache miss trả **cả window + cursor mới** (không slice bằng offset cũ). UI `SupervisedProcessesSection` ưu tiên `details.cursor`, giữ marker `[name: state; cursor=N]` của SDK làm fallback. Test cache **3/3**. **Ruling:** đo before/after dời sang lúc cây sạch (đang có agent sửa file) — số đo live nằm ở commit cuối của plan; cost nếu sai: chưa có số tuyệt đối cho phần `logs` trong bảng #5. |
| 8 | `2c58605` | `exportHtmlFile` (adapter trả **path**, không đọc lại) + route stream `?as=file` (xoá cả temp **dir** sau khi ghi) + route JSON chặn 8 MB → 413; web `OpsBar` tải qua browser, xoá `useExportHtml`/`exportHtml` (dead). Test: route **3/3** (stream nội dung, thiếu file, 413), e2e **1/1** trên session có journal thật, `bun run check` xanh. **Ruling:** bỏ `content-length` khỏi contract (Bun gửi chunked, header bị transport bỏ) → test khẳng định **body**, không header; cost nếu sai: client nào cần size phải tự lấy. **Ruling:** `bytes` trong `exportHtmlFile` bị bỏ vì không nơi nào dùng (dead weight) — cost nếu sai: thêm lại một field khi cần hiển thị size. |
| 9 | `9f88388` | `routes/respond.ts` (gzip > 32 kB + ETag/304) + nhánh `/api/models` dùng etag hash. Đo trên server tươi: **509 853 B → 31 922 B (16×)**, `If-None-Match` → **304**. Test `respond.test.ts` **4/4** (body nhỏ giữ nguyên, gzip + kích thước wire, 304 khi khớp, 200 + etag khi lệch). **Ruling:** bỏ phần `?query=`/`?provider=` + UI lọc của plan — payload là vấn đề thật, lọc là tiện ích UX; cost nếu sai: picker vẫn tải cả catalog một lần (nay 32 kB + cache 304). **Ruling:** gzip áp riêng cho nhánh catalog thay vì bọc mọi `Response.json` (server không có một cửa funnel chung) — cost nếu sai: route lớn khác (nếu có) phải tự gọi `respondJson`. |
| 10 | `f3d9175` | **#12 đóng**: race đã được `runExclusive` (đợt trước) chặn; thêm guard `session-create-race.test.ts` (8 create song song). Chứng minh nhân quả: cùng script **bỏ** `runExclusive` → **7/8 fail** đúng message `Agent "Main" was replaced during session initialization`; giữ nguyên → **6 lần liên tiếp 0 lỗi** (5 lần liên tiếp + 1 lần đầu). Nguyên nhân (SDK-owned): mọi session được đăng ký cùng id `Main` trong `AgentRegistry`, hai cửa sổ init song song ghi đè nhau. Không sửa production code. |
| 11 | _pending_ | **Chốt 2026-09-23** sau khi người dùng cấp quyền macOS. Server cách ly (`PI_CODING_AGENT_DIR` + session tạm), đúng chuỗi lệnh pane gọi: `capabilities` sau run đầu → `{capture:true, input:true, ax:true, backgroundWindowInput:true, deliveryModes:[background,foreground]}`; `displays` → 1 Retina 1680×1050 @2x; `windows` → danh sách thật; `screenshot` → PNG 3360×2100; **input**: `window.screenshot` 396×700 → `click 198,105` 200 → `type "123"` 200 → AX `statictext [ref=e9]: "−123"`; `move` 200; Web `open`→`ariaSnapshot`→`click aria-ref=e2` 200. Parity row `/browser`,`/computer`: 🟡 → **✅**; 4 caveat SDK ghi vào §9. Không đổi production code. |
| 12 | `c1315c5` | `scripts/build-desktop.ts` đọc `GROVE_UPDATER_ENDPOINT` (ghi đúng literal `plugins.updater.endpoints[]`, thiếu env → placeholder + cảnh báo, `--dry-run` chạy không build; test mới trong `scripts/build-desktop.test.ts`). Hai plan desktop có khối "trạng thái thực tế" (đã ship gì / còn mở gì / checkbox cũ không phải nguồn trạng thái); `docs/desktop-release.md` nói rõ endpoint theo env còn notarization vẫn cần credential Apple. **Chưa làm (giữ mở):** notarization thật + chạy migration dir giả (cần mở app GUI) — cost nếu sai: release công khai vẫn bị `spctl` từ chối tới khi có credential. |
...
**Task 11 (ĐÓNG 2026-09-23):** phiên 2026-09-22 không chạy được vì `/computer` đòi **Screen Recording** (+ **Accessibility** cho native input) — người dùng đã cấp; checklist chạy lại trên máy thật và ghi Evidence vào parity row `/browser`,`/computer`, hai dòng đó nay **✅**. Không đổi production code; các ràng buộc SDK gặp khi chạy (3 quyền macOS, `capabilities` trước run đầu, click cần screenshot trước, background keystroke cần app 1 cửa sổ) ghi vào `docs/tui-parity-status.md` §9.
...
- **Xong:** Task 1–10, 12, 13 (Task 11 đóng sau đó ngày 2026-09-23, khi quyền macOS được cấp).
