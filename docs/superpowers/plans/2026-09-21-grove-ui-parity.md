...
| 3 Hairline + elevation | `76229a3` | Contract test mới (utilities + invariant "không còn `shadow-lg/2xl`, `rounded-md border border-border`, `focus-visible:ring-1`"): RED (`Expected to contain: "@utility hairline"`) → **4 pass**; leftover `hsl(var(--` = **0**; probe: panel `border-top 0.5px #e6e6e6`, `shadow-floating` giữ đủ 3 lớp elevation, scrim `rgba(0,0,0,0.4)`, textarea focus `outline 2px solid rgb(60,94,246)` offset 2px. Ruling 6: hairline = **border 0.5px** chứ không phải inset ring — `box-shadow` là nơi elevation sống, ring inset đè mất shadow (probe bắt được: `floatingShadow` = inset hairline). Hệ quả: ở DPR < 2 trình duyệt làm tròn thành 1px (đo ở DPR 1.25), trên màn retina là hairline thật. |

| 4 Kit điều khiển | `a819a72` | Probe TRƯỚC: CTA 32px/500, nút header 28px/12px/500 → SAU: CTA **28px/6px/13px/530**, header 28px/13px/530; tag recipe **16px/2px/11px/530 uppercase** + hairline. 222 `size="sm"` bỏ, Badge `default`→`neutral`, 1 `Button variant="secondary"`→`neutral`. Ruling 7: gộp `Tag` vào `Badge` (một component, không dựng thêm file trùng). Ruling 8: **bug thật do probe bắt** — Tailwind không quét `packages/ui` nên utility chỉ dùng trong kit (`font-strong`, `text-meta`, `hairline`) không được sinh (đo `font-weight` = 440 dù có class); thêm `@source "../../../../packages/ui/src/**/*.{ts,tsx}"`. Cost nếu sai: nếu sau này thêm package dùng class Tailwind thì phải thêm `@source` tương ứng. |

| 5 Dialog primitive | `049b365` | Invariants test `apps/web/src/lib/ui-invariants.test.ts`: RED (dialog.tsx chưa có + 9 wrapper `role="dialog"` + hook còn ở web) → **GREEN 3 pass**; probe: `aria-modal=true`, label "Agent modes", radius 6px, elevation overlay, scrim `rgba(0,0,0,0.4)`, **Escape đóng**. Ruling 1 đã thực thi: hook chuyển vào `packages/ui/src/hooks/`, 6 import đổi sang `@grove/ui`. Ruling 9: `ModelPicker` không dùng `Dialog` (là dropdown neo, không phải modal) → thêm `Popover` (`role=menu|listbox`). Ruling 10: hai banner duyệt inline (`ChatPage` tool approval, `PlanReview`) đổi `role="alertdialog"` → `role="alert"` vì không modal (alertdialog cần focus trap + aria-modal). Ruling 11: test invariant đặt ở `apps/web` chứ không trong `packages/ui` vì tsconfig của package đó không có type của Bun; đổi lại test đọc file chéo package bằng đường dẫn. |

| 6 Panel/state kit | `095d9b1` | Invariant test mở rộng (kit phải tồn tại + không feature nào tự viết lại recipe box lỗi/nhãn): RED (ENOENT `chrome.tsx` + 12 box, 28 nhãn) → **GREEN 5 pass**; đã migrate **12 box lỗi → `ErrorState`** (10 file), **28 nhãn → ramp `text-meta font-strong uppercase`**, **60 dòng lỗi → `text-small`**; probe: nhãn 11px/530, box lỗi radius 6px + hairline, chữ lỗi 12px. Ruling 12: các call site nhãn giữ padding riêng, dùng ramp thay vì `<SectionLabel>` (padding cố định của component sẽ lệch 10+ chỗ); `SectionLabel` là entry cho code mới. |

| 7 Shell/tab/scrollbar | `$(git rev-parse --short HEAD)` | Invariant test mở rộng (scrollbar rule + "mọi `overflow-y-auto` phải có `scroll-area`" + số đo header/row): RED → **GREEN 7 pass**; 46 scroller gắn `scroll-area`; OpsBar + tool header `h-10`; session row `h-7`/radius 6; settings tab 28px/radius 4; probe: row 28px/6px, header 41px, thumb rule phát ra. Ruling 13: **không** thêm utility `tab-strip`/`tab` (plan đề xuất) vì app không có tab ngang — chỉ có rail icon + tab dọc trong Settings; thêm sẽ thành CSS chết. |
