...
/** GET /api/sessions/:id/tools → { tools } (TUI `/tools`). */
export async function sessionToolsRoute(
  runtime: AgentRuntime,
  sessionId: string,
): Promise<{ tools: SessionToolInfo[] }> {
  return { tools: await runtime.getSessionTools(sessionId) };
}
